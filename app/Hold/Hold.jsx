...
const Orders = () => {
  ...
  const fetchDataFromFirestore = async () => {
    ...
    const data = querySnapshot.docs
        .filter(doc => {
            ...
            return (status=='Processing' && (issueNumber === '1' && userId === 'K3ZbPeRwRycr02lK9QNIw7KbHSu1')) ||
                (status=='Processing' && ((issueNumber === '2' || issueNumber === '3') && userId === 'YwbQW9FbP4NziDU1yDOxuAzL2w52')  );
        })
  ...
  const handleCloseTicket = async (orderId) => {
    try {
      ...
      // Refresh data after handling the ticket
  ...
  .sort((a, b) => {
    const timeA = new Date(a.ProcessTime).getTime();
    const timeB = new Date(b.ProcessTime).getTime();
  ...
  return (
    <div className='bg-gray-100 min-h-screen'>
      ...
      {userData.length === 0 ? (
        ...
        <div className='w-full m-auto p-4 border rounded-lg space-y-3 bg-white overflow-y-auto'>
              ...
              >
                <div>
                  ...
                  <div className='text-sm text-gray-500'>
                    ...
                    {new Date(order.ProcessTime).toLocaleString()}
                  </div>
                </div>
